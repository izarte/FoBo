...
class DemoInference:
    ...
    def process_frame_sequence(self, frame):
        self.tracker.reset_siammot_status()
        orig_frame = frame
        results = self.process(frame)
        if self.vis_generator:
            vis_frame, center = self.vis_generator.frame_vis_generator(orig_frame, results)
            return vis_frame, center
